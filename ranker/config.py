...
MODEL_ARCHITECTURE = {
    'model_type', 'embedding_dim', 'hidden_size', 'dim', 'num_layers',
    'question_layers', 'rnn_type', 'concat_rnn_layers', 'question_merge',
    'use_qemb', 'use_in_question', 'use_pos', 'use_ner', 'use_lemma', 'use_question_feature',
}
...
def add_model_args(parser):
    ...
    model.add_argument('--hidden-size', type=int, default=2048,
    ...
    detail.add_argument('--use-question-feature', type='bool', default=False,
                        help='Whether to use term frequency features')

    # Optimization details
    optim = parser.add_argument_group('Model Optimization')
    optim.add_argument('--dropout-emb', type=float, default=0.1,
                       help='Dropout rate for word embeddings')
    optim.add_argument('--dropout-rnn', type=float, default=0.1,
    ...
    optim.add_argument('--weight-decay', type=float, default=0.001,
                       help='Weight decay factor')
    optim.add_argument('--momentum', type=float, default=0.5,
...
def get_model_args(args):  # 返回关于模型的设置
