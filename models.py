...
class BLSTMEncoder(nn.Module):
    ...
    def forward(self, sent_tuple):
        ...
        sent_len = np.ascontiguousarray(sent_len, dtype=np.float32)
    ...
    def build_vocab(self, sentences, tokenize=True):
        assert hasattr(self, 'glove_path'), 'warning : you need \
                                             to set_glove_path(glove_path)'
        word_dict = self.get_word_dict(sentences, tokenize)  # 建立句子中出现的每个词的空字典
        self.word_vec = self.get_glove(word_dict)  # 用glove做句子中出现的词的词向量,每个词300维
    ...
    def get_batch(self, batch):
        ...
        embed = np.zeros((len(batch[0]), len(batch), self.word_emb_dim))  # max_len*batch_size*emb_dim
        # batch[0]为最长的句子
        for i in range(len(batch)):
            for j in range(len(batch[i])):
                embed[j, i, :] = self.word_vec[batch[i][j]]

        return torch.FloatTensor(embed)  # 词向量
    ...
    def prepare_samples(self, sentences, bsize, tokenize, verbose):
        ...
        n_w = np.sum([len(x) for x in sentences])  # 所有句子中单词总数
        ...
        lengths = np.array([len(s) for s in sentences])  # 每个句子的对应长度
        n_wk = np.sum(lengths)  # 总长度
        # if verbose:
        #     print('Nb words kept : {0}/{1} ({2} %)'.format(
        #                 n_wk, n_w, round((100.0 * n_wk) / n_w, 2)))

        # sort by decreasing length
        lengths, idx_sort = np.sort(lengths)[::-1], np.argsort(-lengths)  # 从最长到最短及对应的索引号
    ...
    def encode(self, sentences, bsize=64, tokenize=True, verbose=False):
        ...
        for stidx in tqdm(range(0, len(sentences), bsize),desc="encoding sentence"):  # 生成句子batch
            batch = self.get_batch(sentences[stidx:stidx + bsize]).detach()  # 一个batch句子的词向量
            if self.is_cuda():
                batch = batch.cuda()
            batch = self.forward(
